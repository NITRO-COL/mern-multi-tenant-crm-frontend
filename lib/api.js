...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    /**
     * A blocked CORS response and a dead server look identical to XHR — the
     * browser withholds the response either way — so the message names both
     * rather than asserting the one that is easier to guess wrong.
     */
    ...
    const normalized = new Error(
      payload?.message ||
        (error.code === "ECONNABORTED"
          ...
          : noResponse
            ? "Could not reach the API. It may be starting up, or this origin may not be allowed by its CORS configuration."
